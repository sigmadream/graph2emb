...
class Doc2Vec:
    ...
    def __init__(self, documents: Optional[List[TaggedDocument]] = None,
                 vector_size: int = 100,
                 window: int = 5,
                 min_count: int = 5,
                 workers: int = 3,
    ...
    def build_vocab(self, documents: List[TaggedDocument], update: bool = False):
        ...
        for idx, (word, count) in enumerate(sorted_words):
            ...
            prob = (np.sqrt(count / (self.sample * total_words)) + 1) * \
                   (self.sample * total_words) / count if self.sample > 0 else 1.0
            prob = min(prob, 1.0)
    ...
    def _build_negative_table(self):
        ...
        for i, prob in enumerate(pow_counts):
            cumulative += prob
            while idx < self.negative_table_size and \
                  idx / self.negative_table_size < cumulative:
    ...
    def train(self, documents: List[TaggedDocument], total_examples: Optional[int] = None,
              total_words: Optional[int] = None, epochs: Optional[int] = None,
              start_alpha: Optional[float] = None, end_alpha: Optional[float] = None,
              **kwargs):
        """Train the model."""
        if len(self.vocab) == 0:
        ...
        doc_vectors = np.random.uniform(
            -0.5 / self.vector_size,
            0.5 / self.vector_size,
            (doc_size, self.vector_size)
        ).astype(np.float32)
    ...
    def _train_dm(self, word_indices: List[int], doc_indices: List[int],
                  alpha: float, word_vectors: np.ndarray, doc_vectors: np.ndarray):
        """Train in PV-DM mode."""
    ...
    def _train_dbow(self, word_indices: List[int], doc_indices: List[int],
                    alpha: float, word_vectors: np.ndarray, doc_vectors: np.ndarray):
    ...
    def _train_pair_dm(self, word_idx: int, input_vec: np.ndarray, label: int,
                       alpha: float, word_vectors: np.ndarray):
        """Train PV-DM pair."""
    ...
    def _train_pair_dbow(self, word_idx: int, doc_vec: np.ndarray, label: int,
                         alpha: float, word_vectors: np.ndarray, doc_vectors: np.ndarray,
                         doc_idx: int):
        """Train PV-DBOW pair."""
    ...
    def infer_vector(self, doc_words: List[str], alpha: Optional[float] = None,
                     min_alpha: Optional[float] = None, epochs: Optional[int] = None) -> np.ndarray:
        """
        ...
        doc_vec = np.random.uniform(-0.5 / self.vector_size, 0.5 / self.vector_size,
                                   self.vector_size).astype(np.float32)
